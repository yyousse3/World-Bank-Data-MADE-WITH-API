...
*/
	...
	private void createReport(JPanel west) {
		...
		{
		//	System.out.println("ANFASFKASFAFAFAFAFAFAFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
			//chartPanel = null;
			getContentPane().remove(west);
			west.removeAll();
			activeViewers[5] = false;
	...
	*/
	...
	/*
			...
			if (dataObj.getSeries() >= 1)
			{
				...
				plot.setRangeAxis(new NumberAxis(dataObj.getName()[0]));
			}
			...
			if (dataObj.getSeries() >= 2)
			{
				...
				plot.setRangeAxis(1, new NumberAxis(dataObj.getName()[1]));
			}
...
*/
	...
	*/
	...
	private void createBar(JPanel west) {
		...
		if (dataObj.getSeries() >= 3)
		{
			...
			plot.setRangeAxis(2, new NumberAxis(dataObj.getName()[2]));
		}
	...
	*/
	...
	private void createLine(JPanel west) {
		...
		JFreeChart chart = ChartFactory.createXYLineChart(dataObj.getAnalysis(), "Year", dataObj.getName()[0], dataset,
